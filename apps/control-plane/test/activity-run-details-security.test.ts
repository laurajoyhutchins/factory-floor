...
describe('Activity command-details authorization', () => {
  it('permits only the server-bound command and overrides client identity headers', async () => {
    const app = Fastify();
    ...
    const allowed = await app.inject({
      method: 'GET',
      url: '/api/v1/operator/commands/command-1/details',
      headers: {
    ...
    const denied = await app.inject({
      method: 'GET',
      url: '/api/v1/operator/commands/command-2/details',
      headers: { authorization: 'Bearer activity-session-token' },
    });
    expect(denied.statusCode).toBe(403);
    expect(denied.json()).toMatchObject({
      error: { code: 'activity_command_binding_mismatch' },
    });
    await app.close();
  });
});