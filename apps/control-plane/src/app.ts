...
import { CommandService, RegistrationService, SystemApplicationService } from '@factory-floor/runtime-core';
...
export interface AppDependencies { database?: Kysely<Database>; registrationService?: RegistrationService; systemApplicationService?: SystemApplicationService; commandService?: CommandService; }
export async function buildApp(deps: AppDependencies = {}): Promise<FastifyInstance> {
  ...
  if (db || deps.registrationService || deps.systemApplicationService) {
    ...
    await registerRegistrationRoutes(app, reg); await registerSystemRoutes(app, sys); await registerCommandRoutes(app, deps.commandService ?? new CommandService(db!));
