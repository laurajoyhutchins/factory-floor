...
const portfolioClient = portfolioBaseUrl ? createPortfolioClient({ baseUrl: portfolioBaseUrl }) : undefined;
const titles: Record<string, string> = {
  portfolio: 'Portfolio', topology: 'Topology', executions: 'Executions', artifacts: 'Artifacts', instantiations: 'Template instantiations', operations: 'Operations', approvals: 'Pending approvals', commands: 'Command inspection',
};

function CommandRoute() {
  const { commandId = '' } = useParams();
  return <><CommandOperatorWorkspace commandId={commandId} /><RunDetailsPanel runId={commandId} loadDetails={(id) => runDetailsClient.getRunDetails(id)} /></>;
}

function App() {
  const live = useLiveEvents(50);
  const location = useLocation();
  const health = useQuery({ queryKey: ['health'], queryFn: ({ signal }) => consoleApi.health(signal) });
  const segment = location.pathname.split('/').filter(Boolean)[0];
  const title = segment ? (titles[segment] ?? 'Not found') : 'Overview';
  const healthStatus = health.data?.status ?? (health.isError ? 'disconnected' : 'checking');
  return <Shell title={title} live={live.state} controlPlane={healthStatus} lastRefreshed={health.dataUpdatedAt || undefined}>
    <Routes>
      <Route path="/" element={<Overview healthStatus={healthStatus} liveEvents={live.events} liveState={live.state} />} />
      ...
      <Route path="/instantiations/:instantiationId" element={<TemplateInstantiationDetail />} />
