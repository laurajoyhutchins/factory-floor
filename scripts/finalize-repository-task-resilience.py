...
path = Path("examples/repository-task/run-dogfood.ts")
text = path.read_text()

old_import = """  CommandService,
  RegistrationService,
  SystemApplicationService,
"""
new_import = """  CommandService,
...
if old_import not in text:
    raise SystemExit("runtime-core import marker not found")
text = text.replace(old_import, new_import, 1)

marker = "    const unavailableRevision = 'f'.repeat(40);\n"
block = r'''    const retryCommand = await commandService.submit({
...
summary_marker = "      deliberateFailure: {\n"
summary_block = r'''      retry: {
...
text = text.replace(summary_marker, summary_block + summary_marker, 1)
