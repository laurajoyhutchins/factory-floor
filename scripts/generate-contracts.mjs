...
const schemaPaths = readdirSync(schemaDir, { recursive: true })
...
for (const schemaPath of schemaPaths) {
  ...
  if (typeof schema.title === 'string') {
    exports.push(
      `export type { ${schema.title} } from './generated/${base.replaceAll('/', '-')}.js';`,
    );
  }
}
writeFileSync(
  join(root, 'packages', 'contracts-ts', 'src', 'index.ts'),
  `${exports.join('\n')}\n`,
);
...
execFileSync(
  ...
  [
    'run',
    '--project',
    join(root, 'packages', 'contracts-py'),
    '--locked',
    'datamodel-codegen',
    '--input',
    schemaDir,
...
const pyModules = schemaPaths.map(
  (schemaPath) =>
    relative(schemaDir, schemaPath)
      .replace(/\.schema\.json$/, '')
      .replaceAll('/', '.')
      .replaceAll('-', '_') + '_schema',
);
writeFileSync(
...
globalThis.console.log(
  `Generated ${schemaPaths.length} schemas from sorted paths:\n${schemaPaths.map((p) => `- ${relative(root, p)}`).join('\n')}`,
);
