...
function task(objective: string) {
  return {
    ...
    authority: { mayCreateBranch: true, mayOpenDraftPullRequest: true, mayMerge: false as const },
  };
}
...
describe('operator command service', () => {
  ...
  it('uses the accepted command id as the run id', async () => {
    const receipt = await commands.submitDevelopmentTask(operator, task('Run identity'));
  ...
  it('records and replays equivalent approval decisions', async () => {
    ...
    await expect(commands.decideApproval(operator, approvalId, request))
      .resolves.toMatchObject({ disposition: 'accepted' });
    await expect(commands.decideApproval(operator, approvalId, request))
      .resolves.toMatchObject({ disposition: 'replayed' });
    await expect(commands.decideApproval(operator, approvalId, { ...request, decision: 'reject' }))
      .rejects.toBeInstanceOf(OperatorConflictError);
  });

  it('cancels only the selected run and fences its stale attempt', async () => {
    const first = await commands.submitDevelopmentTask(operator, task('Cancel this run'));
    ...
    const request = { clientRequestId: createUuidV7(), reason: 'Operator requested cancellation.' };
    await expect(commands.cancelRun(operator, first.runId, request)).resolves.toMatchObject({
      disposition: 'accepted',
      cancelledExecutions: 1,
      cancelledAttempts: 1,
    });
    await expect(commands.cancelRun(operator, first.runId, request))
      .resolves.toMatchObject({ disposition: 'replayed' });

    const firstAttempt = await db.selectFrom('execution_attempts').select('status')
      .where('id', '=', firstClaim.envelope.attemptId).executeTakeFirstOrThrow();
    const secondAttempt = await db.selectFrom('execution_attempts').select('status')
      .where('id', '=', secondClaim.envelope.attemptId).executeTakeFirstOrThrow();
    expect(firstAttempt.status).toBe('cancelled');
    expect(secondAttempt.status).toBe('leased');

    await expect(workers.submitResult({
      protocolVersion: '1.0',
