...
`componentSelectors` in `WorkerClaimRequest` is the canonical allowlist of component implementations available in that worker process. The v1 `capabilities` field is retained as a deprecated compatibility name for the same selector list. The canonical selector is the registered component definition `name@version`, for example `retrieve@1`. An empty list claims no work. The scheduler applies the filter before leasing, so an incompatible worker never takes ownership of an attempt.
