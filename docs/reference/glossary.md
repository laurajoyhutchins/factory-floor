...
Public v1 wire names and existing database columns may retain older compatibility names; adapters translate them at module boundaries.
...
- **Component selector**: `name@version` selector for a component definition a worker can execute, such as `retrieve@1`.
- **Capability**: governed runtime authority for external or privileged actions, represented by definitions, grants, handles, policies, and authorization decisions.
...
- **Trigger delivery**: selected delivery stored in `executions.delivery_id`; complete execution inputs are stored in `execution_inputs`.
...
- **State**: durable lifecycle value of an entity, such as execution state or artifact-staging state.
...
- **Projection checkpoint**: durable cursor for a read-model projection stream; advancing a checkpoint is distinct from rebuilding projection data.

## Do not confuse

- Component selectors match worker implementations; capabilities authorize governed external authority.
- An execution is the durable unit of work; an attempt is one leased try to complete it.
- Delivery leases protect queue dispatch; attempt leases protect worker execution and result submission.
- `artifact_staging.id` identifies a database row; `artifact_staging.staged_ref` is the worker-visible protocol reference.
- Lifecycle status describes state; the region fencing epoch prevents stale work from committing after cancellation or replacement.
- Checkpoint advancement records consumed stream position; rebuilding reconstructs projection state from source records.
