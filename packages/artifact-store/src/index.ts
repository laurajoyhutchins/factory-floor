...
export class FilesystemArtifactBlobStore implements ArtifactBlobStore {
  ...
  async stage(
    ...
    const temporaryDirectory =
      await this.createTemporaryObjectDirectory('stage');
    ...
    try {
      ...
      if (isAlreadyPublished(error, 'staging_conflict')) {
        const existing = await this.readVerifiedObjectMetadata(
          finalDirectory,
          'staging_conflict',
        );
  ...
  async promote(
    ...
    const stagedMetadata = await this.readObjectMetadata(
      stagedDirectory,
      'staging_conflict',
    );
    ...
    const temporaryDirectory =
      await this.createTemporaryObjectDirectory('commit');
    try {
  ...
  async removeStaged(stagingId: StagingId): Promise<void> {
    validateStagingId(stagingId);
    await rm(this.stagedObjectDirectory(stagingId), {
      recursive: true,
      force: true,
    });
  }
  ...
  async listStaged(options: ListStagedOptions): Promise<StagedObjectPage> {
    ...
    const slice = ids.slice(
      start < 0 ? ids.length : start,
      (start < 0 ? ids.length : start) + options.limit,
    );
    const objects: StagedObject[] = [];
    for (const id of slice) {
      const directory = this.stagedObjectDirectory(id);
      const metadata = await this.readVerifiedObjectMetadata(
        directory,
        'not_found',
      );
      const stat = await lstat(directory);
      objects.push({
        ...this.stagingReceipt(id, metadata.digest, metadata.size),
        lastModifiedAt: stat.mtime,
      });
    }
    const last = slice.at(-1);
    const nextCursor =
      last !== undefined && ids.some((id) => id > last) ? last : undefined;
  ...
  private async publishObjectDirectory(
    ...
    const temporary = await this.readVerifiedObjectMetadata(
      temporaryDirectory,
      conflictCode,
    );
    if (temporary.digest !== digest || temporary.size !== size) {
      throw new ArtifactBlobStoreError(
        'temporary object identity does not match requested content',
        conflictCode,
      );
    }
    try {
      ...
      if (existing.digest !== digest || existing.size !== size) {
        throw new ArtifactBlobStoreError(
          'object identity conflicts with existing content',
          conflictCode,
        );
      }
  ...
  private async validObjectExists(
    ...
    try {
      ...
      if (error instanceof ArtifactBlobStoreError && error.code === 'not_found')
        return false;
      throw error;
    }
  }
  ...
  private async readVerifiedObjectMetadata(
    ...
    const measured = await measureFileNoFollow(
      join(directory, 'data'),
      missingCode,
    );
    if (
      measured.size !== metadata.size ||
      measured.digest !== metadata.digest
    ) {
      throw new ArtifactBlobStoreError(
        'object content does not match its metadata',
        missingCode,
      );
    }
    return metadata;
  ...
  private async ensureCommittedPrefix(digest: string): Promise<void> {
    ...
    await this.ensureRootDirectory(
      join(this.committedRoot, 'sha256', digest.slice(0, 2)),
    );
  }

  private async ensureRootDirectory(directory: string): Promise<void> {
    const relative = resolve(directory)
    ...
    if (!isWithin(actualRoot, actualDirectory)) {
      throw new ArtifactBlobStoreError(
        'storage directory escapes configured root',
        'unsafe_path',
      );
    }
  }
  ...
  private stagingReceipt(
    ...
    return {
      ...
      digest,
      size,
      stagedLocator: `file:staging/${stagingId}`,
    };
  }
...
export class S3ArtifactBlobStore implements ArtifactBlobStore {
  ...
  async stage(
    ...
    try {
      ...
      await this.putObject(
        finalKey,
        await this.getReadable(tmpKey, 'staging_conflict'),
        measured,
        true,
      );
      return this.stagingReceipt(stagingId, measured.digest, measured.size);
      ...
      if (isS3PreconditionFailed(error)) {
        const existing = await this.readVerifiedMetadata(
          finalKey,
          'staging_conflict',
        );
        if (
  ...
  async promote(
    ...
    const staged = await this.readVerifiedMetadata(
      stagedKey,
      'staging_conflict',
    );
    if (staged.size !== size)
    ...
    try {
      await this.putObject(
        committedKey,
        await this.getReadable(stagedKey, 'staging_conflict'),
        { digest, size },
        true,
      );
    } catch (error) {
  ...
  async stagedExists(stagingId: StagingId): Promise<boolean> {
    validateStagingId(stagingId);
    return this.validObjectExists(
      this.stagingKey(stagingId),
      undefined,
      undefined,
      'not_found',
    );
  ...
  async listStaged(options: ListStagedOptions): Promise<StagedObjectPage> {
    ...
    for (const item of response.Contents ?? []) {
      ...
      objects.push({
        ...this.stagingReceipt(stagingId, metadata.digest, metadata.size),
        lastModifiedAt: item.LastModified ?? new Date(0),
      });
    }
  ...
  private async readVerifiedMetadata(
    key: string,
    missingCode: ArtifactBlobStoreError['code'],
  ): Promise<ObjectMetadata> {
  ...
  private async getReadable(
    ...
    try {
      ...
      if (isS3NotFound(error))
        throw new ArtifactBlobStoreError('object not found', missingCode);
      throw error;
    }
  }
  ...
  private async validObjectExists(
    ...
    try {
      ...
      if (error instanceof ArtifactBlobStoreError && error.code === 'not_found')
        return false;
      if (
        conflictCode === 'not_found' &&
        error instanceof ArtifactBlobStoreError &&
  ...
  private stagingReceipt(
    ...
    return {
      ...
      digest,
      size,
      stagedLocator: this.locator(this.stagingKey(stagingId)),
    };
  }
...
class AlreadyPublishedError extends Error {
  constructor(
    readonly conflictCode: 'staging_conflict' | 'committed_conflict',
  ) {
...
function isAlreadyPublished(
  ...
  return (
    ...
    error.conflictCode === conflictCode
  );
}
...
function isValidStagingId(stagingId: string): boolean {
  return (
    /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(stagingId) &&
    !stagingId.includes('..')
  );
}
...
function safeJoin(root: string, ...parts: string[]): string {
  ...
  if (!isWithin(root, path)) {
    throw new ArtifactBlobStoreError(
      'resolved path escaped storage root',
      'unsafe_path',
    );
  }
  return path;
...
async function assertDirectoryNoFollow(
  ...
  try {
    ...
    if (!stats.isDirectory()) {
      throw new ArtifactBlobStoreError(
        'expected directory without symlink',
        missingCode,
      );
    }
  } catch (error) {
...
async function assertRegularNoFollow(
  path: string,
  missingCode: ArtifactBlobStoreError['code'],
): Promise<void> {
  try {
    ...
    if (!stats.isFile()) {
      throw new ArtifactBlobStoreError(
        'expected regular file without symlink',
        missingCode,
      );
    }
  } catch (error) {
...
async function openReadableNoFollow(
  path: string,
  missingCode: ArtifactBlobStoreError['code'],
): Promise<Readable> {
  try {
...
async function readMetadata(
  ...
  try {
    ...
    if (code === 'ENOENT' || code === 'ELOOP' || code === 'ENOTDIR') {
      throw new ArtifactBlobStoreError(
        'object metadata not found or unsafe',
        missingCode,
      );
    }
    throw error;
...
function parseS3Metadata(
  ...
  if (
    ...
    throw new ArtifactBlobStoreError(
      'object metadata format is missing or unsupported',
      missingCode,
    );
  }
