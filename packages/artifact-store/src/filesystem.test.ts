...
describe('FilesystemArtifactBlobStore filesystem safety and atomicity', () => {
  ...
  it('retries promotion after committed publication but before staged cleanup', async () => {
    ...
    const committedDirectory = join(root, 'committed', 'sha256', staged.digest.slice(0, 2), staged.digest);
    await mkdir(committedDirectory, { recursive: true });
    await writeFile(join(committedDirectory, 'data'), 'abc');
    await writeFile(join(committedDirectory, 'metadata.json'), JSON.stringify({ digest: staged.digest, size: staged.size.toString() }) + '\n');
    await expect(store.promote('left-behind', staged.digest, staged.size)).resolves.toMatchObject({ digest: staged.digest, size: staged.size });
  ...
  it('does not treat pre-existing incomplete staging objects as valid', async () => {
    ...
    await expect(store.readStaged('incomplete')).rejects.toMatchObject({ code: 'not_found' });
    await expect(store.stage('incomplete', chunks('abc'))).rejects.toMatchObject({ code: 'staging_conflict' });
  });

  it('does not treat pre-existing incomplete committed objects as valid', async () => {
    const digest = digestOf('abc');
    await mkdir(join(root, 'committed', 'sha256', digest.slice(0, 2), digest), { recursive: true });
    await writeFile(join(root, 'committed', 'sha256', digest.slice(0, 2), digest, 'data'), 'abc');
    expect(await store.committedExists(digest)).toBe(false);
    await expect(store.readCommitted(digest)).rejects.toMatchObject({ code: 'not_found' });
    const staged = await store.stage('source', chunks('abc'));
    await expect(store.promote('source', staged.digest, staged.size)).rejects.toMatchObject({ code: 'committed_conflict' });
  });
  ...
  it('removes temporary directories after successful promotion and handled failures', async () => {
    ...
    expect(await tmpEntries(root)).toEqual([]);
    await expect(store.stage('cleanup-failure', chunks('abc'), { expectedSize: 999n })).rejects.toMatchObject({ code: 'size_mismatch' });
    expect(await tmpEntries(root)).toEqual([]);
  });
  ...
  it('rejects symlinked staging namespace directories', async () => {
    ...
    await expect(store.stage('escape', chunks('abc'))).rejects.toMatchObject({ code: 'unsafe_path' });
  });
  ...
  it('rejects symlinked committed prefix directories', async () => {
    ...
    await symlink(join(root, 'outside-prefix'), join(root, 'committed', 'sha256', staged.digest.slice(0, 2)));
    await expect(store.promote('prefix-escape', staged.digest, staged.size)).rejects.toMatchObject({ code: 'unsafe_path' });
  });
  ...
  it('does not follow symlink escapes for final staged data files', async () => {
    ...
    await symlink(join(root, 'outside'), join(root, 'staging', 'escape', 'data'));
    await writeFile(join(root, 'staging', 'escape', 'metadata.json'), JSON.stringify({ digest: digestOf('secret'), size: '6' }) + '\n');
    await expect(store.readStaged('escape')).rejects.toMatchObject({ code: 'not_found' });
  });
});
