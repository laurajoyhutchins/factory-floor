...
import { ArtifactBlobStoreError, FilesystemArtifactBlobStore, type ArtifactBlobStore } from './index.js';
...
function artifactBlobStoreConformance(createStore: (root: string) => ArtifactBlobStore) {
  let root: string;
  ...
  it('does not follow symlink escapes for staged reads', async () => {
    await mkdir(join(root, 'staging'), { recursive: true });
    await writeFile(join(root, 'outside'), 'secret');
    await symlink(join(root, 'outside'), join(root, 'staging', 'escape.blob'));
