...
const TERMINAL_EXECUTION_STATUSES = ['completed', 'failed'];
...
const OPERATOR_CANCELLATION_FAILURE = {
  code: 'operator_cancelled',
  message: 'Execution was cancelled by an operator.',
  retryable: false,
} satisfies Json;

type CancellationRun = {
  id: string;
  region_id: string;
...
export class OperatorCommandService {
  ...
  async decideApproval(
    ...
    return this.db.transaction().execute(async (transaction) => {
      ...
      return approvalReceipt(approvalId, context, input, reason, 'accepted');
    });
  }
  ...
  async cancelRun(
    ...
    return this.db.transaction().execute(async (transaction) => {
      ...
      const cancellationRun: CancellationRun = {
        id: run.id,
      ...
      await this.events.insert(transaction, {
        ...
        payload: { runId, reason, clientRequestId: input.clientRequestId },
      ...
      const cancellableAttemptIds = attempts
        .filter(
          (attempt) => !TERMINAL_ATTEMPT_STATUSES.includes(attempt.status),
        )
      ...
      if (cancellableExecutionIds.length)
        await transaction
          ...
          .set({
            ...
            failed_at: now,
            completed_at: null,
            failure: OPERATOR_CANCELLATION_FAILURE,
          })
  ...
  private async cancellationReceipt(
    ...
    return {
      ...
      cancelledExecutions: executions.filter((execution) =>
...
function requireClientRequestId(value: string) {
  if (!value?.trim())
    throw new OperatorValidationError('client_request_id_required');
}
...
function validateDevelopmentTask(input: DevelopmentTaskRequest) {
  ...
  if (input.authority?.mayMerge === true)
...
function isOperatorCancellation(value: Json | null) {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    value.code === OPERATOR_CANCELLATION_FAILURE.code
  );
}
