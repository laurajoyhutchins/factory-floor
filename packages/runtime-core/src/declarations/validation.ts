...
function requireReference(value: unknown, label: string): string {
  if (!nonEmptyString(value) || !/^[^@]+@[^@]+$/.test(value)) {
    throw new DomainError('invalid_declaration', `${label} must be a name@version reference`);
  }
  return value;
}
...
function requireEndpoint(value: unknown, label: string): string {
  if (!nonEmptyString(value) || !/^[^.]+\.[^.]+$/.test(value)) {
    throw new DomainError('invalid_declaration', `${label} must be an instance.port endpoint`);
  }
  return value;
}
...
export function requireEnvelope(doc: any, kind?: string): void {
  if (!doc || typeof doc !== 'object' || Array.isArray(doc)) {
    throw new DomainError('invalid_declaration', 'Declaration must be an object');
  }
  if (![apiVersion, legacyApiVersion].includes(doc.apiVersion)) {
    throw new DomainError('unsupported_declaration_version', 'Unsupported apiVersion');
  }
  if (kind && doc.kind !== kind) throw new DomainError('invalid_declaration', `Expected kind ${kind}`);
  if (!nonEmptyString(doc.metadata?.name) || !nonEmptyString(doc.metadata?.version)) {
    throw new DomainError('invalid_declaration', 'metadata.name and metadata.version must be non-empty strings');
  }
}
...
export function validateArtifactSchemaDeclaration(doc: any): void {
  ...
  if (schema.$schema !== 'https://json-schema.org/draft/2020-12/schema') {
    throw new DomainError('invalid_declaration', 'Artifact schemas must use JSON Schema Draft 2020-12');
  }
  try {
    new Ajv2020({ strict: true }).compile(schema);
  } catch (error) {
    throw new DomainError('invalid_declaration', `Invalid JSON Schema: ${(error as Error).message}`);
  }
}
...
export function validateComponentDefinitionDeclaration(doc: any): void {
  ...
  for (const port of doc.spec.ports) {
    if (
      ...
      throw new DomainError('invalid_declaration', 'Each port requires name, direction, required');
    }
    const reference = port.schema ?? port.schemaRef;
    if (!nonEmptyString(reference?.name) || !nonEmptyString(reference?.version)) {
      throw new DomainError('invalid_declaration', 'Each port requires schema natural key');
    }
    const key = `${port.name}:${port.direction}`;
    if (seen.has(key)) throw new DomainError('duplicate_port_identity', `Duplicate port ${key}`);
    seen.add(key);
  }
}
...
export function validateStaticTopology(topology: any): void {
  if (!topology || typeof topology !== 'object' || Array.isArray(topology)) {
    throw new DomainError('invalid_declaration', 'initialTopology must be an object');
  }
  if (!Array.isArray(topology.instances) || !Array.isArray(topology.connections)) {
    throw new DomainError('invalid_declaration', 'initialTopology requires instances and connections arrays');
  }

  const names = new Set<string>();
  for (const instance of topology.instances) {
    if (!nonEmptyString(instance?.name)) {
      throw new DomainError('invalid_declaration', 'Each topology instance requires a non-empty name');
    }
    requireReference(instance.component, `Component reference for ${instance.name}`);
    if (names.has(instance.name)) {
      throw new DomainError('duplicate_component_instance', `Duplicate component instance ${instance.name}`);
    }
    ...
    if (
      ...
      throw new DomainError('invalid_declaration', `Configuration for ${instance.name} must be an object`);
    }
  }
...
export function validateSimpleDeclaration(doc: any, kind: 'Template' | 'Policy'): void {
...
export function validateSystemDeclaration(doc: any): void {
  ...
  if (!nonEmptyString(spec.rootRegion?.id)) {
    throw new DomainError('invalid_declaration', 'spec.rootRegion.id is required');
  }
  if (!Array.isArray(spec.regions) || spec.regions.length === 0) {
    throw new DomainError('invalid_declaration', 'spec.regions must contain at least one stable region');
  }

  const regionIds = new Set<string>();
  for (const region of spec.regions) {
    if (!nonEmptyString(region?.id)) throw new DomainError('invalid_declaration', 'Each region requires an id');
    requireReference(region.template, `Template reference for region ${region.id}`);
    if (regionIds.has(region.id)) throw new DomainError('duplicate_region_identity', `Duplicate region ${region.id}`);
    regionIds.add(region.id);
  }

  if (spec.connections !== undefined) {
    if (!Array.isArray(spec.connections)) throw new DomainError('invalid_declaration', 'spec.connections must be an array');
