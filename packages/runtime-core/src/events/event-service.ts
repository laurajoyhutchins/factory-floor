...
export class EventService {
  ...
  async insert(
    ...
    input: {
      ...
      sourceComponentInstanceId?: string | null;
      sourcePortName?: string | null;
    },
  ) {
    if (input.sourceKind === 'component' && !input.sourcePortName)
      throw new Error('component events require sourcePortName');
    ...
    return db
      ...
      .values({
        ...
        source_command_id: input.sourceCommandId ?? null,
        source_event_id: input.sourceEventId ?? null,
        source_execution_id: input.sourceExecutionId ?? null,
        source_attempt_id: input.sourceAttemptId ?? null,
        source_component_instance_id: input.sourceComponentInstanceId ?? null,
        source_port_name: input.sourcePortName ?? null,
      } as any)
      .returningAll()
