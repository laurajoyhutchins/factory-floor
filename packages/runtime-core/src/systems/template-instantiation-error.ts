...
export const templateInstantiationErrorCodes = [
  'invalid_declaration',
...
export type TemplateInstantiationErrorCode =
  (typeof templateInstantiationErrorCodes)[number];
...
const exposedDomainCodes = new Set<TemplateInstantiationErrorCode>(
  templateInstantiationErrorCodes.filter(
    (code) => code !== 'internal_transient_failure',
  ),
);
...
export function toTemplateInstantiationError(
  ...
  if (
    isDomainError(error) &&
    exposedDomainCodes.has(error.code as TemplateInstantiationErrorCode)
  ) {
    return {
      protocolVersion: '1.0',
